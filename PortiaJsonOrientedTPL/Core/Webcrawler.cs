...
using PortiaTask = PortiaJsonOrientedTPL.Core.Models.Task;
using PortiaResponse = PortiaJsonOrientedTPL.Core.Dtos.Response;
using PortiaRequest = PortiaJsonOrientedTPL.Core.Dtos.Request;
using System.Text;
using PortiaJsonOrientedTPL.Core;
using System.Reactive.Disposables;

namespace PortiaJsonOrientedTPL
{
    public class Webcrawler
    {
        ...
        public async Task CreateBlocks()
        {
            ...
            htmlDownloader = new TransformBlock<Uri, UrlHtmlPair>(async url =>
            {
            ...
            objParser = new ActionBlock<UrlHtmlPair>(urlHtmlPair => ParseObjects(urlHtmlPair));
            urlParser = new TransformManyBlock<UrlHtmlPair, Uri>(urlHtmlPair =>
            {
                var newUrls = ParseUrls(urlHtmlPair.Html);
                return newUrls;
            });
        ...
        public async Task<PortiaResponse> StartCrawlerAsync(PortiaRequest request)
        {
            ...
            if (allUnfinishedWork.Count() != 0)
            {
                ...
                unfinishedUrls.ForEach(async (uu) => await htmlDownloader.SendAsync(uu));
            }
            else
            {
                await htmlDownloader.SendAsync(rootUrl);
            }
            #endregion
        ...
        private static void ParseObjects(UrlHtmlPair urlHtmlPair)
        {
            HtmlDocument htmlDoc = new HtmlDocument();
